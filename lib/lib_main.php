...
function get_task_list() {
    global $pdo_db;
    $out = array();
    $res = $pdo_db->query("SELECT contest_id, contest_name FROM contests WHERE parent_id = 0 ORDER BY contest_id");
    //collect tasks
    foreach ($res as $r) {
        $contest = array('id' => $r['contest_id'], 'name' => $r['contest_name']);
        $res1 = $pdo_db->query("SELECT task_id, task_name, status FROM tasks WHERE contest_id = ".$r['contest_id']." ORDER BY task_id");
        ...
        $res1 = $pdo_db->query("SELECT contest_id, contest_name FROM contests WHERE parent_id = ".$r['contest_id']." ORDER BY contest_id");
        foreach ($res1 as $r1) {
            $round = array('id' => $r1['contest_id'], 'name' => $r1['contest_name']);
            $res2 = $pdo_db->query("SELECT task_id, task_name FROM tasks WHERE contest_id = ".$r1['contest_id']." ORDER BY task_id");
            foreach ($res2 as $r2) {
                $round['tasks'][] = array(
                    'id' => $r2['task_id'],
                    'name' => $r2['task_name'],
...
function get_judges_for_task($task_id) {
    ...
    $res = $pdo_db->query("SELECT * FROM judges WHERE judge_id IN (SELECT judge_id FROM judges_by_tasks WHERE task_id = $task_id)");
    $out = array();
    foreach ($res as $r) {
...
function get_contest_by_task($task_id, $highest = false) {
    global $pdo_db;
    $res = $pdo_db->query("SELECT contest_id FROM tasks WHERE task_id = $task_id LIMIT 1");
    ...
    $pid = $r['contest_id'];
    while ($pid > 0) {
        $tr = $pdo_db->query("SELECT parent_id FROM contests WHERE contest_id = $pid LIMIT 1");
        $r = $tr->fetchAll()[0];
...
function get_temporary_marks($task_id, $judge_id) {
    global $pdo_db;
    $marks = array();
    $res = $pdo_db->query("SELECT solution_id, code FROM solutions_tmp WHERE task_id=$task_id AND judge_id=$judge_id");
    foreach ($res as $r) {
        $res1 = $pdo_db->query("SELECT mark_id, subtask_id, mark_value FROM marks_tmp WHERE solution_id=".$r['solution_id']." ORDER BY subtask_id");
        $t = array();
        foreach ($res1 as $r1) {
...
function save_temporary_marks($task_id, $judge_id, $marks) {
    ...
    $pdo_db->query("START TRANSACTION");
    $ret_codes = array();
    foreach($marks as $solution) {
        ...
        foreach($solution['marks'] as $subtask_id => $value) {
            ...
            $value = (float)$value;
            $res = $pdo_db->query("SELECT mark_id FROM marks_tmp WHERE solution_id=".$solution['id']." AND subtask_id=$subtask_id LIMIT 1");
            if ($res->rowCount() > 0) {
                $r = $res->fetchAll()[0];
...
function delete_temporary_solution($solution_id) {
    ...
    if (!$pdo_db->query("DELETE FROM solutions_tmp WHERE solution_id = $solution_id LIMIT 1")) return false;
    return true;
}
function get_aggregate_marks($task_id) {
    ...
    $aggr_codes = array();
    //already aggregated
    $res = $pdo_db->query("SELECT solution_id, s.contestant_id, code FROM solutions s LEFT JOIN contestants USING(contestant_id) WHERE task_id = $task_id ORDER BY code");
    foreach ($res as $r) {
...
function save_aggregate_string($task_id, $contestant_id, $code, $marks) {
    ...
    $res = $pdo_db->query("SELECT solution_id FROM solutions WHERE task_id = $task_id AND contestant_id = $contestant_id LIMIT 1");
    if ($res->rowCount() == 0) {
    ...
    $old_marks = array();
    $res = $pdo_db->query("SELECT mark_id, subtask_id FROM final_marks WHERE solution_id = $solution_id");
    foreach ($res as $r) {
...
function get_task_result($task_id) {
    ...
    $res = $pdo_db->query("select contestant_id, ceil(sum(mark_value)) as mark from solutions left join final_marks using(solution_id) where task_id = $task_id group by solution_id");
    $solutions = array();
    foreach ($res as $r) {
